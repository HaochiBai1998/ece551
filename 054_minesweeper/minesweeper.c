...
board_t * makeBoard(int w, int h, int numMines) {
  board_t *b=NULL;
  b=malloc(sizeof(*b));
  //initBoard->width=malloc(sizeof(initBoard->width));
  //initBoard->height=malloc(sizeof(initBoard->height));
  //initBoard->totalMines=malloc(sizeof(initBoard->totalMines));
  b->width=w;
  b->height=h;
...
int countMines(board_t * b, int x, int y) {
  //WRITE ME!
  int ans=0;
  int offset_x[3]={-1,0,1};
  int offset_y[3]={-1,0,1};
  for (int i=0;i<3;i++){
    for (int j=0;j<3;j++){
      if(b->width>x+offset_x[i]&&x+offset_x[i]>=0&&
	 b->height>y+offset_y[j]&&y+offset_y[j]>=0&&
	 !(offset_x[i]==0&&offset_y[j]==0))
	{
	  if (IS_MINE(b->board[y+offset_y[j]][x+offset_x[i]]))
	    ans++;
	}
    }
  }
